...
LittleRoot::LittleRoot(Renderer* r, DynamicObject* d)
{
	...
	// add events
	m_events.insert(std::pair<std::string, Event*>(WarpID::LITTLEROOT_EXTERIOR_HOSPITAL_WARP, new WarpEventInside(Sublocation_List::Test_Hospital_Interior, { 736, 512 })));
	m_events.insert(std::pair<std::string, Event*>(WarpID::LITTLEROOT_INTERIOR_HOSPITAL_WARP_EXIT, new WarpEventOutside({ 704, 512 }, d)));
